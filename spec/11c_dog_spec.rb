...
describe Dog do
  ...
  subject(:thumper) { described_class.new(name: 'thumper', color: 'brown') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when instance is a child of Pet class' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(thumper).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(thumper).to be_sleeping
  end
end
